...
class OneMessageMod(loader.Module):
    ...
    async def watcher(self, message):
        ...
        if message.message and (
            message.raw_text[0]
            in self._db.get("friendly-telegram.modules.corectrl", "command_prefix", ".")
            or message.fwd_from
        ):
            return
        if (
            self._db.get("OneMessage", "status", None)
            and message.sender_id == self._db.get("OneMessage", "my_id", None)
